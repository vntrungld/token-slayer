...
--}}
<div
    x-data="flairPreview(@js($color), @js($durationMs), @js($label))"
    x-init="start()"
...
<script>
    function flairPreview(initialColor, initialDurationMs, label) {
        return {
            color: initialColor || '#fbbf24',
            durationMs: initialDurationMs || 6000,
            label: (label || 'MODEL').toUpperCase(),
            angle: 0,
            lastFrameAt: null,
            ...
            frame(now) {
                ...
                // Matches the real Phaser burst: its objects are created
                // (and thus display-list-inserted) AFTER the fighter
                // container already exists, so they render on top of it
                // regardless of nominal depth ties.
